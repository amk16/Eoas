...
export default function LiveScribeSilence({
  ...
  const checkPartialTranscriptPattern = (newPartialTranscript: string) => {
    ...
    if (partialTranscriptsRef.current.length === 3) {
      ...
      if (allSame) {
        ...
        logger.debug('Silence pattern detected: 3 identical partial transcripts', {
  ...
  const sendConfiguration = (ws: WebSocket) => {
    ...
    try {
      ...
      logger.debug('Configuration message sent successfully');
    } catch (err: any) {
      logger.error('Failed to send configuration message', err);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (messageType === 'session_started') {
            ...
            logger.debug('Configuration update received from server', data);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (messageType === 'session_started') {
            ...
            logger.debug('Configuration update received from server (fallback)', data);
          } else if (messageType === 'error' || messageType?.endsWith('Error')) {
